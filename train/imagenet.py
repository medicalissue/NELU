...
# Gate Normalization arguments (layered on top of upstream timm defaults).
group = parser.add_argument_group('Gate Normalization')
group.add_argument('--activation', default='gelu', type=str,
...
group.add_argument('--log-gate-probe-size', type=int, default=16,
                   help='Batch size used for the gate-stats probe forward pass.')
...
def main():
    ...
    if utils.is_primary(args):
        ...
        if args.log_wandb:
            if has_wandb:
                # Persist the W&B run id in a sidecar JSON so a resumed job
                # (e.g. after a spot-instance preemption) reconnects to the
                # same run without requiring --wandb-resume-id.
                sidecar = os.path.join(output_dir, 'wandb_run_id.json')
                run_id = args.wandb_resume_id or None
                if run_id is None and os.path.isfile(sidecar):
                    try:
                        with open(sidecar) as f:
                            run_id = json.load(f).get('run_id') or None
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if args.log_gate_stats_every > 0 and (epoch % args.log_gate_stats_every == 0):
                ...
                if args.activation in ("nelu", "nilu"):
                    gn_metrics.update(gamma_stats(model))
                if loader_eval is not None:
                    try:
                        probe = next(iter(loader_eval))[0][: args.log_gate_probe_size]
                        gn_metrics.update(gate_stats(model, probe, device))
